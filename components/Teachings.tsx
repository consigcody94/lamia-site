        ...
        <div className="mb-16 text-center">
          ...
          <h2 className="mt-4 font-display text-4xl font-semibold tracking-wide md:text-5xl">
            <span
              ...
              style={{
                backgroundImage:
                  "linear-gradient(180deg, #f5ecd7 0%, #e8a8a8 40%, #b84040 100%)",
              }}
            >
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {TEACHINGS.map((t, i) => (
            <motion.article
              ...
              className="group relative flex flex-col overflow-hidden rounded-xl border border-red-950/40 bg-gradient-to-br from-black/60 via-red-950/15 to-black/60 p-6 backdrop-blur transition hover:border-[#b84040]/50"
            >
              <div
                className="pointer-events-none absolute inset-0 opacity-0 transition duration-700 group-hover:opacity-100"
                style={{
                  background:
                    "radial-gradient(circle at 50% 0%, rgba(139,0,0,0.2), transparent 60%)",
                }}
              />
              <div
                className="relative mb-4 flex h-12 w-12 items-center justify-center rounded-full border text-2xl leading-none"
                style={{
                  borderColor: "rgba(184,64,64,0.45)",
                  color: "#b84040",
                  background: "radial-gradient(circle, rgba(139,0,0,0.25), transparent 70%)",
                  textShadow: "0 0 16px rgba(139,0,0,0.8)",
                }}
              >
                {t.glyph}
              </div>
              <h3 className="relative font-display text-xl font-semibold tracking-wide text-[#f5ecd7]">
                {t.title}
              </h3>
              <p className="relative mt-1 font-serif text-xs italic uppercase tracking-[0.25em]" style={{ color: "#b84040" }}>
                {t.epithet}
              </p>
              <p className="relative mt-4 font-serif text-sm leading-relaxed text-[#f5ecd7]/75">
                {t.body}
              </p>
            </motion.article>
          ))}
